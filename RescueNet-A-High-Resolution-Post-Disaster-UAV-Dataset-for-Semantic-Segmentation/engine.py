    ...
    for i in range(pred_rgb.shape[0]):
        ...
        if combine:
            ...
            for im, im_dir in zip(imgs, (save_dir / "input", save_dir / "pred", save_dir / "gt")):
                ...
                im_dir.mkdir(exist_ok=True)
                (im_dir / save_name).parent.mkdir(parents=True, exist_ok=True)
...
def eval_dataset(
    ...
    if ptu.dist_rank == 0:
        ...
        with open(model_dir / f"scores_{suffix}.yml", "w") as f:
            f.write(scores_str)
        if wandb:
            shutil.copy(str(model_dir / f"scores_{suffix}.yml"), os.path.join(WandB.run.dir, f"scores_{suffix}.yml"))
