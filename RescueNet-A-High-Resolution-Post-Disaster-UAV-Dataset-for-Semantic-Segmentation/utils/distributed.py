...
def init_process(backend="nccl"):
    ...
    '''
    print("Running in non-distributed (single-GPU or debug) mode.", flush=True)
    # Override default distributed state
    ptu.dist_rank = 0
    ptu.world_size = 1

    # Optionally suppress output from non-rank-0 processes (not needed now)
    silence_print(True)
